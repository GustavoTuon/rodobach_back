...
for(const file of (await fs.readdir(directory)).sort()) {
  ...
  if(!match||match[1]!==match[2]||+match[1]<17||+match[1]>21||match[3]!=='20260922')continue;
...
try {
 ...
 try {
  ...
  ({rows:database}=await client.query(`SELECT v.placa,t.id,t.data_referencia::text AS dia,t.distancia,t.consumo_total_litros,t.consumo_total,t.media_consumo,t.hodometro_ini,t.hodometro_fim,t.created_at,t.updated_at
    ...
    WHERE t.data_referencia BETWEEN $1::date AND $2::date AND v.placa=ANY($3::text[]) ORDER BY t.updated_at DESC NULLS LAST,t.id DESC`,['2026-09-17','2026-09-21',[...new Set(selected.map(row=>row.placa))]]));
  ({rows:freshness}=await client.query(`SELECT v.placa,MAX(t.data_referencia)::text AS ultima_data,MAX(t.updated_at) AS ultima_atualizacao,
    COUNT(*) FILTER(WHERE t.data_referencia BETWEEN '2026-09-17' AND '2026-09-21')::int AS registros_periodo
    FROM ${quoteIdent(schema)}.telemetria_relatorio t JOIN ${quoteIdent(schema)}.veiculos v USING(veiculo_id)
...
await fs.mkdir(output,{recursive:true});await fs.writeFile(path.join(output,'consumo-telemetria-auditoria-2026-09-22.json'),JSON.stringify(result,null,2));
const columns=Object.keys(comparison[0]||{});const cell=value=>`"${String(value??'').replaceAll('"','""')}"`;
await fs.writeFile(path.join(output,'consumo-telemetria-comparativo-2026-09-22.csv'),'\uFEFF'+[columns,...comparison.map(row=>columns.map(key=>row[key]))].map(row=>row.map(cell).join(';')).join('\r\n'));
